...
const uploadImage = asyncHandler( async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: 'Image uploaded successfully', 'url': result.url , 'public_id': result.public_id });
  } catch (error) {
...
const getImageById = asyncHandler( async (req, res) => {
  ...
  return res.status(200).json({'url': result.url , 'public_id': result.public_id });
    ...
    return res.status(500).json({ error: "Failed to get image", error: error });
  }
});
